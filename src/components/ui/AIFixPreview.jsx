export default function AIFixPreview() {
  return (
    <div className="w-full max-w-[480px] bg-white dark:bg-charcoal rounded-2xl border border-gray-100 dark:border-white/10 shadow-soft overflow-hidden select-none">
      {/* Issue bar */}
      <div className="px-5 py-3.5 flex items-start gap-3 border-b border-gray-100 dark:border-white/10">
        <div className="mt-1 w-2 h-2 rounded-full bg-coral flex-shrink-0" />
        <div className="flex-1 min-w-0">
          <p className="text-sm font-semibold text-ink dark:text-white leading-tight">Missing alt attribute</p>
...
/>`}</pre>
      ...
      {/* Footer */}
      <div className="px-5 py-3 bg-ivory dark:bg-night/40 border-t border-gray-100 dark:border-white/10">
        <p className="text-xs text-body dark:text-gray-400 mb-2 leading-relaxed">
          Screen readers need descriptive text to convey image purpose to users who cannot see it.
        </p>
        <div className="flex items-center gap-2">
          <span className="text-[10px] text-body dark:text-gray-500">Available for</span>
          {['HTML', 'React', 'Vue'].map((f) => (
            <span
              key={f}
              className="text-[10px] font-semibold text-ink dark:text-gray-300 bg-white dark:bg-white/10 border border-gray-200 dark:border-white/10 px-1.5 py-0.5 rounded"
            >
              {f}
            </span>
          ))}
