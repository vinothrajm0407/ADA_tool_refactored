  ...
  return (
    <div className="space-y-3">
      {entries.map(([moduleId, state]) => (
        <div key={moduleId} className="bg-white rounded-2xl border border-gray-100 overflow-hidden">
  ...
  return (
    <div className="flex items-center gap-5 mt-1">
      <svg viewBox="0 0 100 100" className="w-28 h-28 flex-shrink-0" role="img" aria-label="Severity distribution donut chart">
        ...
        <text x="50"y="46"textAnchor="middle"fontSize="15"fontWeight="700"fill="currentColor"className="text-ink">{total}</text>
        <text x="50"y="59"textAnchor="middle"fontSize="8"fill="currentColor"className="text-body">issues</text>
      </svg>
            ...
            <div key={key} className="space-y-0.5">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="text-[10px] text-body">{sublabel}</span>
                </div>
                <span className="text-xs font-bold text-ink">{count}</span>
              </div>
              <div className="h-1.5 bg-gray-100 rounded-full overflow-hidden ml-3.5">
  ...
  if (!prevScan) return (
    <div className="bg-white rounded-2xl border border-gray-100 p-6">
      <div className="flex items-center gap-2 mb-2">
        <span className="font-heading font-semibold text-base leading-none text-ink">Comparison</span>
        <span className="text-[10px] font-semibold uppercase tracking-wide text-body bg-gray-100 px-2 py-0.5 rounded-full">No prior scan</span>
      </div>
    ...
    <div className="bg-white rounded-2xl border border-gray-100 p-6">
      <div className="flex items-center gap-2 mb-4">
        ...
        {prevScan.url && prevScan.url !== scanUrl && (
          <span className="text-[10px] text-body font-mono truncate max-w-[180px]"title={prevScan.url}>
            vs {prevScan.url}
          </span>
        )}
      </div>
      <div className="grid grid-cols-3 gap-3 text-center">
        <div className="bg-gray-50 rounded-xl p-3">
          <p className="text-[10px] font-bold uppercase tracking-widest text-body mb-2">Previous</p>
          <p className="text-2xl font-heading font-bold text-body">{prevScan.totalViolations}</p>
          <p className="text-[11px] text-body mt-0.5">violations</p>
          <p className="text-xs font-semibold text-body mt-1">{prevScan.score}/100</p>
        </div>
        <div className="flex flex-col items-center justify-center gap-1">
          <p className={`text-2xl font-heading font-bold ${violDiff < 0 ?'text-sage': violDiff > 0 ?'text-coral':'text-body'}`}>
            {violDiff > 0 ? '+' : ''}{violDiff}
          </p>
          <p className="text-[10px] text-body">violations</p>
          <p className={`text-xs font-semibold ${scoreDiff > 0 ?'text-teal': scoreDiff < 0 ?'text-coral':'text-body'}`}>
            {scoreDiff !== 0 ? `${scoreDiff > 0 ? '+' : ''}${scoreDiff} score` : 'No change'}
          </p>
        </div>
        <div className="bg-gray-50 rounded-xl p-3">
          <p className="text-[10px] font-bold uppercase tracking-widest text-body mb-2">Current</p>
          <p className="text-2xl font-heading font-bold text-ink">{currentTotal}</p>
          <p className="text-[11px] text-body mt-0.5">violations</p>
          <p className="text-xs font-semibold text-body mt-1">{currentScore}/100</p>
        </div>
      </div>
      {violDiff < 0 && (
...
function IssueNavigator({ sevBreak, totalViolations, onNavigate }) {
  ...
  return (
    <nav aria-label="Issue navigator"className="bg-white rounded-2xl border border-gray-100 p-4">
      <p className="text-[10px] font-bold uppercase tracking-widest text-body mb-3">Navigate</p>
      <div className="space-y-0.5">
        <button
          type="button"
          onClick={() => onNavigate('all')}
          className="w-full flex items-center justify-between px-2.5 py-2 rounded-lg text-xs hover:bg-teal/5 transition-colors text-left"
        >
          <span className="font-semibold text-ink">All Issues</span>
          <span className="font-bold text-body">{totalViolations}</span>
        </button>
        {NS_SEVERITY_ROWS.map(({ key, label, dot }) => {
          ...
          return (
            <button
              ...
              className="w-full flex items-center gap-2 px-2.5 py-2 rounded-lg text-xs hover:bg-gray-50 transition-colors text-left"
            >
    ...
    <div className="bg-white rounded-2xl border border-gray-100 p-6">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <span className="font-heading font-semibold text-base leading-none text-ink">Accessibility Readiness</span>
        </div>
      ...
      <div className="grid grid-cols-4 gap-3 mb-4">
        <div className="bg-gray-50 rounded-xl p-3 text-center">
          <p className="text-[10px] font-bold uppercase tracking-widest text-body mb-1">Score</p>
          <p className="text-xl font-heading font-bold text-ink">{score}<span className="text-xs font-normal text-body">/100</span></p>
        </div>
        <div className="bg-gray-50 rounded-xl p-3 text-center">
          <p className="text-[10px] font-bold uppercase tracking-widest text-body mb-1">WCAG</p>
          <p className="text-sm font-heading font-bold text-ink leading-tight mt-1">{wcag}</p>
        </div>
        <div className="bg-gray-50 rounded-xl p-3 text-center">
          <p className="text-[10px] font-bold uppercase tracking-widest text-body mb-1">Critical</p>
          <p className={`text-xl font-heading font-bold ${criticalCount > 0 ? 'text-coral' : 'text-sage'}`}>{criticalCount}</p>
        </div>
        <div className="bg-gray-50 rounded-xl p-3 text-center">
          <p className="text-[10px] font-bold uppercase tracking-widest text-body mb-1">Pass Rate</p>
          <p className="text-xl font-heading font-bold text-ink">{passRate}<span className="text-xs font-normal text-body">%</span></p>
        </div>
      </div>
      <div className="bg-teal/5 border border-teal/15 rounded-xl px-4 py-3">
        ...
        <p className="text-sm text-ink font-medium">{risk.action}</p>
      </div>
    </div>
  );
}
    ...
    <div className="bg-white rounded-2xl border border-gray-100 p-6">
      <div className="flex items-center justify-between mb-1">
        <div className="flex items-center gap-2">
          ...
          <span className="font-heading font-semibold text-base leading-none text-ink">Viewport Comparison</span>
        </div>
        <span className="text-[10px] font-semibold text-body bg-gray-100 px-2 py-0.5 rounded-full">Beta</span>
      </div>
      ...
      <div className="space-y-2">
        {rows.map(vp => (
          <div key={vp.id} className="flex items-center gap-3 px-3 py-2.5 rounded-xl bg-gray-50">
            <div className="w-16 flex-shrink-0">
              <p className="text-xs font-semibold text-ink">{vp.label}</p>
              <p className="text-[10px] text-body">{vp.width}px</p>
            </div>
            {vp.data ? (
              <div className="flex-1 flex items-center gap-2 min-w-0">
                <span className="text-sm font-bold text-ink">{vp.data.violations}</span>
                <span className="text-xs text-body truncate">violations · {vp.data.score}/100</span>
                ...
                {!vp.isBase && vp.data.violations === currentTotal && (
                  <span className="ml-auto text-[10px] text-body flex-shrink-0">No diff</span>
                )}
              </div>
            ) : (
              <button
...
export default function ScanResults({ violations, incomplete, passes, scanRan, scanUrl, onReset, screenshot, screenshotType, sessionId }) {
  ...
  return (
    <div className={`space-y-4 transition-all duration-300 ${appeared ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-1'}`}>
      ...
      {!revealDone && (
        <div className={`bg-white rounded-2xl border border-teal/25 px-6 py-6 transition-opacity duration-500 ${revealVisible ?'opacity-100':'opacity-0'}`}>
          <div className="flex items-center justify-center gap-2.5 mb-5">
            ...
            <span className="font-heading font-semibold text-base text-ink">Scan Complete</span>
            {scanRan && (
              <span className="text-xs text-body">
                &nbsp;· Accessibility Score&nbsp;<span className="font-heading font-bold text-ink">{score}</span>
              </span>
            )}
          </div>
          <div className="grid grid-cols-3 gap-3 max-w-sm mx-auto">
            <div className={`rounded-xl px-4 py-4 text-center ${violations.length > 0 ?'bg-coral/10':'bg-gray-50'}`}>
              <p className={`font-heading font-bold text-3xl leading-none ${violations.length > 0 ?'text-coral':'text-body'}`}>{violCount}</p>
              <p className="text-[11px] text-body mt-1.5 font-medium">Violations</p>
            </div>
            <div className={`rounded-xl px-4 py-4 text-center ${incomplete.length > 0 ?'bg-amber/10':'bg-gray-50'}`}>
              <p className={`font-heading font-bold text-3xl leading-none ${incomplete.length > 0 ?'text-amber':'text-body'}`}>{incompleteCount}</p>
              <p className="text-[11px] text-body mt-1.5 font-medium">Needs Review</p>
            </div>
            <div className="rounded-xl px-4 py-4 text-center bg-gray-50">
              <p className="font-heading font-bold text-3xl leading-none text-sage">{passCount}</p>
              <p className="text-[11px] text-body mt-1.5 font-medium">Passed</p>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="bg-white rounded-2xl border border-gray-100 p-6">
        <div className="flex items-start justify-between gap-4 mb-4">
          <div>
            ...
            <p className="text-xs font-mono text-body break-all max-w-sm">{scanUrl}</p>
          </div>
        ...
        <div className="grid grid-cols-3 gap-3">
          {violations.length > 0 ? (
            <button
              ...
              className="rounded-xl px-4 py-3 text-center bg-coral/10 hover:bg-coral/[0.18] transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-teal focus-visible:ring-offset-1"
            ...
            >
              ...
              <p className="text-[11px] text-body mt-1 font-medium">Violations</p>
            </button>
          ) : (
            <div className="rounded-xl px-4 py-3 text-center bg-gray-50">
              <p className="font-heading font-bold text-2xl leading-none text-body">{violCount}</p>
              <p className="text-[11px] text-body mt-1 font-medium">Violations</p>
            </div>
          )}
          {incomplete.length > 0 ? (
            <button
              ...
              className="rounded-xl px-4 py-3 text-center bg-amber/10 hover:bg-amber/[0.18] transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-teal focus-visible:ring-offset-1"
            ...
            >
              ...
              <p className="text-[11px] text-body mt-1 font-medium">Needs Review</p>
            </button>
          ) : (
            <div className="rounded-xl px-4 py-3 text-center bg-gray-50">
              <p className="font-heading font-bold text-2xl leading-none text-body">{incompleteCount}</p>
              <p className="text-[11px] text-body mt-1 font-medium">Needs Review</p>
            </div>
          )}
          <div className="rounded-xl px-4 py-3 text-center bg-gray-50">
            <p className="font-heading font-bold text-2xl leading-none text-sage">{passCount}</p>
            <p className="text-[11px] text-body mt-1 font-medium">Passed</p>
          </div>
        </div>
        {!scanRan && (
      ...
      {hasPageScreenshot && (
        <div className="bg-white rounded-2xl border border-gray-100 p-6">
          <p className="font-heading font-semibold text-base text-ink mb-1">Page screenshot</p>
          <p className="text-xs text-body mb-3">View the page as it was when the accessibility check ran.</p>
          <button
            type="button"
      ...
      {scanRan && (
        <div className="grid grid-cols-2 gap-4">
          <div className="bg-white rounded-2xl border border-gray-100 p-6">
            <p className="text-[11px] font-semibold uppercase tracking-widest text-body mb-4">
              Accessibility Score
            </p>
            <div className="flex items-center gap-4">
              ...
              <div className="min-w-0">
                ...
                <p className="text-sm text-body leading-snug">{nsScoreMessage(score)}</p>
              </div>
            </div>
          </div>
          <div className="bg-white rounded-2xl border border-gray-100 p-6">
            <p className="text-[11px] font-semibold uppercase tracking-widest text-body mb-1">
      ...
      {topIssues.length > 0 && (
        <div className="bg-white rounded-2xl border border-gray-100 p-6">
          <p className="font-heading font-semibold text-base text-ink mb-4">Top Issues</p>
          <div className="space-y-2">
            {topIssues.map((issue) => (
              <button
                key={issue.id}
                ...
                className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl bg-gray-50 hover:bg-teal/5 transition-colors text-left"
              >
                <span className="flex-1 text-sm font-medium text-ink truncate min-w-0">{issue.title}</span>
                <span className="w-24 h-1.5 bg-gray-200 rounded-full overflow-hidden flex-shrink-0">
      ...
      {violations.length > 0 && (
        <div id="ns-violations-section" className="flex gap-4 items-start">
          <div className={`flex-1 min-w-0 bg-white rounded-2xl border p-6 transition-all duration-500 ${
            ...
            <div className="flex items-start justify-between mb-4 gap-3 flex-wrap">
              <p className="font-heading font-semibold text-base text-ink">
                Violations{' '}
              ...
              <div className="flex items-center gap-2 flex-wrap">
                <div className="relative">
                  ...
                  <input
                    ...
                    className="pl-7 pr-3 py-1 text-xs rounded-lg border border-gray-200 bg-white text-ink placeholder-gray-400 focus:outline-none focus:border-teal w-36 transition-colors"
                  />
                </div>
                {['all', 'A', 'AA', 'BP'].map(lvl => (
                  <button
            ...
            <div className="flex items-center gap-2 flex-wrap mb-4">
              <div className="flex items-center bg-gray-100 rounded-lg p-0.5 border border-gray-200">
                <button
                  ...
                  className={`px-2.5 py-1 rounded-md text-xs font-medium transition-colors ${groupMode ==='rule'?'bg-white text-teal':'text-body hover:text-ink'}`}
                >
                  By Rule
                </button>
                <button
                  ...
                  className={`px-2.5 py-1 rounded-md text-xs font-medium transition-colors ${groupMode ==='area'?'bg-white text-teal':'text-body hover:text-ink'}`}
                >
                  By Area
                </button>
              </div>
              <button
                ...
                className={`text-xs font-semibold px-3 py-1 rounded-full border transition-colors ${filterImpact ==='all'?'bg-ink text-white border-ink':'border-gray-200 text-body hover:border-gray-400'}`}
              >
                All
              </button>
            ...
            {finalViolations.length === 0 ? (
              <div className="text-center py-8">
                <p className="text-sm text-body">No violations match your filters.</p>
                {hasActiveFilter && (
                ...
                {areaGroups.map(({ area, violations: aViolations }) => (
                  <div key={area}>
                    <div className="flex items-center gap-2 mb-2 px-1">
                      <p className="text-[10px] font-bold uppercase tracking-widest text-body">{area}</p>
                      <span className="text-[10px] font-semibold text-body bg-gray-100 px-1.5 py-0.5 rounded-full">{aViolations.length}</span>
                    </div>
                    <div className="space-y-2">
                      {aViolations.map((v, i) => (
      ...
      {/* Needs Review */}
      {incomplete.length > 0 && (
        <div id="ns-needs-review-section"className={`bg-white rounded-2xl border p-6 transition-all duration-500 ${
          highlightSection === 'needs-review'
            ?'border-teal/40 ring-1 ring-teal/20'
            :'border-gray-100'
        }`}>
          <div className="flex items-center gap-2 mb-1">
            <span className="font-heading font-semibold text-base text-ink">Needs Review</span>
      ...
      {scanRan && scanUrl && (
        <div className="space-y-3">
          <div className="bg-white rounded-2xl border border-gray-100 p-5 flex items-center justify-between gap-4">
            <div>
              <p className="font-heading font-semibold text-sm text-ink mb-0.5">Deepen your audit</p>
              <p className="text-xs text-body">Check keyboard navigation, color contrast, and more with Assistive Testing.</p>
            </div>
      ...
      {violations.length === 0 && incomplete.length === 0 && scanRan && (
        <div className="bg-white rounded-2xl border border-gray-100 p-10 text-center">
          ...
          <p className="font-heading font-bold text-2xl text-ink mb-3">Excellent accessibility</p>
          ...
          <p className="text-sm text-body max-w-sm mx-auto leading-relaxed mb-6">
